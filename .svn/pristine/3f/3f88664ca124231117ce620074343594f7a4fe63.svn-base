...
/**
 *  Show list of disbursed task for grid for Customer
 *  For details go through Use-Case doc named 'ExhShowDisbursedTaskForCustomerActionService'
 */
class ExhShowDisbursedTaskForCustomerActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            List<ExhTask> exhTasks = listDisbursedTask()     // list of task
            int total = countDisbursedTask()
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            ...
            List<ExhTask> taskList = wrapTasksForCustomer(exhTasks, start)
            Map gridObject = [page: pageNumber, total: total, rows: taskList]
    ...
    private List<ExhTask> wrapTasksForCustomer(List<ExhTask> lstTask, int start) {
        ...
        for (int i = 0; i < lstTask.size(); i++) {
            ...
            obj.cell = [counter, task.id, task.currentStatus, createDate, task.refNo,
                    task.amountInForeignCurrency, amountGBP, totalDue,
                    task.beneficiaryName, payMethod, taskPinNo]
            tasks << obj
            counter++
        }
        return tasks
    ...
    /**
     * Get list of task which status is SENT_TO_BANK, SENT_TO_OTHER_BANK, RESOLVED_BY_OTHER_BANK
     */
    private List<ExhTask> listDisbursedTask() {
        ...
        return ExhTask.findAllByCompanyIdAndCustomerIdAndCurrentStatusInList(
                companyId,
    ...
    /**
     * Get count of task which status is SENT_TO_BANK, SENT_TO_OTHER_BANK, RESOLVED_BY_OTHER_BANK
     */
    private int countDisbursedTask() {
        long companyId = exhSessionUtil.appSessionUtil.getCompanyId()
        SystemEntity exhSentToBankSysEntityObject = (SystemEntity) exhTaskStatusCacheUtility.readByReservedAndCompany(exhTaskStatusCacheUtility.STATUS_SENT_TO_BANK, companyId)
        SystemEntity exhSentToOtherBankSysEntityObject = (SystemEntity) exhTaskStatusCacheUtility.readByReservedAndCompany(exhTaskStatusCacheUtility.STATUS_SENT_TO_OTHER_BANK, companyId)
        SystemEntity exhResolvedByOtherBankSysEntityObject = (SystemEntity) exhTaskStatusCacheUtility.readByReservedAndCompany(exhTaskStatusCacheUtility.STATUS_RESOLVED_BY_OTHER_BANK, companyId)

        return ExhTask.countByCompanyIdAndCustomerIdAndCurrentStatusInList(
                companyId,
                exhSessionUtil.getUserCustomerId(),
                [exhSentToBankSysEntityObject.id,
                        exhSentToOtherBankSysEntityObject.id,
