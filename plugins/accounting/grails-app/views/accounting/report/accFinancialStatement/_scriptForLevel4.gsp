<script type="text/javascript">
    ...
    jQuery(function ($) {
        ...
        $('#printFinancialStatementPdf').click(function () {
            printFinancialStatementListOfLevel4('pdf');
            return false;
        });

        $('#printFinancialStatementCsv').click(function () {
            printFinancialStatementCsvOfLevel4();
            return false;
        });
        onLoadFinancialStatementOfLevel4();
    });

    function printFinancialStatementListOfLevel4(formatType) {
        var confirmMsg;
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadFinancialStatementOfLevel4')}" + params;
    ...
    function printFinancialStatementCsvOfLevel4() {
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadFinancialStatementCsvOfLevel4')}" + params;
            document.location = url;
        }
        showLoadingSpinner(false);
        return true
    ...
    function onLoadFinancialStatementOfLevel4() {
        try {   // Model will be supplied to grid on load _list.gsp
            $('.download_icon_set').hide();
            initializeForm($("#searchForm"), getFinancialStatementListOfLevel4);
        ...
        loadNumberedMenu(MENU_ID_ACCOUNTING, "#accReport/showFinancialStatementOfLevel4");
    }

    function getFinancialStatementListOfLevel4() {
        ...
        showLoadingSpinner(true);
        $.ajax({
            url: "${createLink(controller:'accReport', action: 'listFinancialStatementOfLevel4')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId,
    ...
    function populateFinancialStatementListGrid(data) {
        ...
        var financialStatementListUrl = "${createLink(controller:'accReport', action: 'listFinancialStatementOfLevel4')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId;
        ...
        (
                {
                    ...
                    colModel: [
                        ...
                        {display: "COA ID/Tier3 ID", name: "coaId", width: 30, sortable: false, align: "right", hide: "true"},
                    ...
                    usepager: false,
                    singleSelect: true,
                    title: 'Financial Statement (Hierarchy-4)',
                    useRp: true,
                    rp: 20,
