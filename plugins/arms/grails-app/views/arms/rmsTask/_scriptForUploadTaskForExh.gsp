<script type="text/javascript">
    var dropDownPaymentMethod, validatorTaskForm,pagerErrors,kendoListViewError;
    ...
    function onLoadUploadTask() {
        ...
        kendoListViewError =  $("#lstViewTaskErrors").kendoListView({
            dataSource: new kendo.data.DataSource({ data: []}),
            template: "<li class='list-group-item list-group-item-danger'>#:id#. #:name#</li>"
        }).data("kendoListView");
        $("#taskFile").kendoUpload({multiple: false});

        $(document).attr('title', "ARMS - Upload Task");
        loadNumberedMenu(MENU_ID_ARMS, "#rmsTask/showForUploadTaskForExh");
