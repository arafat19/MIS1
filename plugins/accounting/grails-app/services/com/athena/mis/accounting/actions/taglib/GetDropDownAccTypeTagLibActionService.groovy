...
/*Renders html of 'select' for AccType objects */
class GetDropDownAccTypeTagLibActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            List<AccType> lstAccType = accTypeCacheUtility.list()
            String html = buildDropDown(lstAccType, dropDownAttributes)
            return html
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstAccType - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<AccType> lstAccType, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstAccType = Tools.listForKendoDropdown(lstAccType, null, hintsText)  // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstAccType as JSON
