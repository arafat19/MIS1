...
/**
 *  Update transport cost object and grid data
 *  For details go through Use-Case doc named 'UpdateTransportCostActionService'
 */
class UpdateTransportCostActionService extends BaseService implements ActionIntf {
    private final Logger log = Logger.getLogger(getClass())

    private static final String CURRENT_USER = "currentUser"
    private static final String TRANSPORT_COST_UPDATE_FAILURE_MESSAGE = "Transport Cost could not be updated"
    private static final String TRANSPORT_COST_UPDATE_SUCCESS_MESSAGE = "Transport Cost has been updated successfully"
    ...
    private static final String NOT_EDITABLE_MSG = "The PO has been sent for approval, make PO editable to update transport cost"
    ...
    /**
     * Get new transport cost object for update
     * 1. Check input validation error by params id & version
     * 2. Get oldTransportCost from transportCostService
     * 3. Check the existence of oldTransportCost
     * 4. Get purchaseOrder object from purchaseOrderService
     * 5. Check the existence of purchaseOrder
     * 6. if purchase order is sent for approval then transport cost can not be updated
     * 7. Build newTransportCost using buildTransportCostObject() method
     * @param parameters - serialized parameters from UI
     * @param obj - N/A
     * @return - a map containing transportCost object, purchase order object & isError(true/false) depending on method success
     */
    ...
    public Object executePreCondition(Object parameters, Object obj) {
        ...
        try {
            ...
            ProcTransportCost oldTransportCost = (ProcTransportCost) transportCostService.readByIdAndVersion(transportCostId, version)
            if (!oldTransportCost) {
                result.put(Tools.MESSAGE, TRANSPORT_COST_NOT_FOUND)
                return result
            }
            ProcPurchaseOrder purchaseOrder = purchaseOrderService.read(oldTransportCost.purchaseOrderId)
            if (!purchaseOrder) {
            ...
            ProcTransportCost newTransportCost = buildTransportCostObject(params, oldTransportCost)
            purchaseOrder.trCostTotal = purchaseOrder.trCostTotal + newTransportCost.amount - oldTransportCost.amount
            purchaseOrder.totalPrice = purchaseOrder.totalPrice + newTransportCost.amount - oldTransportCost.amount

            result.put(TRANSPORT_COST_OBJ, newTransportCost)
            ...
            result.put(Tools.IS_ERROR, Boolean.TRUE)
            result.put(Tools.MESSAGE, TRANSPORT_COST_UPDATE_FAILURE_MESSAGE)
            return result
        }
    }

    /**
     * Update transport cost object in DB
     * 1. Get transport cost object
     * 2. Get purchaseOrder object
     * 3. Update transport cost by using transportCostService
     * 4. Update purchase order for transport cost by updateForTransportCostUpdate method
     * @param parameters - serialized parameters from UI
     * @param obj - map from executePreCondition method
     * @return - a map containing isPOApproved, currentUser, purchaseOrder object, transportCost object &
     */
    @Transactional
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            updateForTransportCostUpdate(purchaseOrder)
            ...
            log.error(ex.getMessage())
            //@todo:rollback
            throw new RuntimeException("Failed to update Transport Cost")
            result.put(Tools.IS_ERROR, Boolean.TRUE)
            result.put(Tools.MESSAGE, TRANSPORT_COST_UPDATE_FAILURE_MESSAGE)
            return result
        }
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            ...
            result.put(Tools.MESSAGE, TRANSPORT_COST_UPDATE_SUCCESS_MESSAGE)
     ...
     * @param oldTransportCost - object of old transport cost
     * @return - transport cost object
     */
    private ProcTransportCost buildTransportCostObject(GrailsParameterMap parameterMap, ProcTransportCost oldTransportCost) {
        ProcTransportCost transportCost = new ProcTransportCost()
        transportCost.id = oldTransportCost.id
        transportCost.version = oldTransportCost.version
        transportCost.purchaseOrderId = oldTransportCost.purchaseOrderId
        ...
        transportCost.rate = parameterMap.rate ? Double.parseDouble(parameterMap.rate.toString()) : 0.0d
     ...
     */
    private int updateForTransportCostUpdate(ProcPurchaseOrder purchaseOrder) {
        Map queryParams = [
