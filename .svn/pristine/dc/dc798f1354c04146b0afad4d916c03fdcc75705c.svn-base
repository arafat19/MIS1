...
/**
 *   Search task details through refNo or pinNo for Admin
 *  For details go through Use-Case doc named 'ExhGetDetailsByRefOrPinActionService'
 */
class ExhGetDetailsByRefOrPinActionService extends BaseService implements ActionIntf {
     ...
     * 4. check companyId of both task & login user
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            SystemEntity taskStatus = exhTaskStatusCacheUtility.read(task.currentStatus)
    ...
    private ExhTask searchTaskByPinNo(String securityNo, Date createdDateFrom, Date createdDateTo) {
        ...
        ExhTask exhTask = ExhTask.findByPinNoAndCreatedOnBetween(securityNo, createdDateFrom, createdDateTo, [readOnly: true]) // need companyId
        return exhTask
    }
    ...
    private ExhTask searchTaskByRefNo(String securityNo, Date createdDateFrom, Date createdDateTo) {
        securityNo = securityNo.toUpperCase()
        ExhTask exhTask = ExhTask.findByRefNoAndCreatedOnBetween(securityNo, createdDateFrom, createdDateTo, [readOnly: true])  // need companyId
        return exhTask
    }
    ...
    private TaskDetailsMap getTaskDetailsFromSFSLBeforeSentToBank(TaskQuery taskQuery) {
        ...
        try {
            ...
            buildTaskDetailsMapForSFSLBeforeSentToBank(taskDetailsMap, result)
    ...
    /**
     *  build taskDetailsMap for SFSL
     * @param taskDetailsMap
     * @param result -an object of GroovyRowResult
     */
