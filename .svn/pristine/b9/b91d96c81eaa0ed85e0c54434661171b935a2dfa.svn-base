<script type="text/javascript">
    ...
    function onLoadSanction() {
        initSanctionGrid();
        var customerName = modelJsonForSanction.customerName ? modelJsonForSanction.customerName :'';
        $("#customerName").val(customerName);
        $("#createdOn").mask("99/99/9999");
        $("#lastUpdate").mask("99/99/9999");
        // update page title
        $('span.headingText').html('Sanction Information');
        $('#icon_box').attr('class', 'pre-icon-header my_task_list');
        $(document).attr('title', "ARMS - Sanction Information");
//        loadNumberedMenu(MENU_ID_EXCHANGE_HOUSE, "#exhSanction/showFromCustomer");
        loadFlexGridSanction();
    }
...
//                            {name: 'Delete', bclass: 'delete', onpress : deleteBank},
    ...
    function loadFlexGridSanction() {
        var params = "?customerName=" + $("#customerName").val();
        var strUrl = "${createLink(controller: 'exhSanction', action: 'listForCustomer')}" + params;
