...
/**
 *  Class to get search list of inventory item(s) received from supplier
 *  For details go through Use-Case doc named 'SearchForItemReceivedStockActionService'
 */
class SearchForItemReceivedStockActionService extends BaseService implements ActionIntf {
    ...
    private static final String INVALID_INPUT_MESSAGE = "Can't not search item received stock due to invalid input"
    ...
    private static final String ITEM_RECEIVED_STOCK_NOT_FOUND = "Item Received Stock not found within given date range"
    ...
    /**
     * method to get search list of inventory item(s) received from supplier
     * @param parameters -parameters from UI
     * @param obj -N/A
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            //search item-received-list-from-supplier
            LinkedHashMap itemReceivedQtyMap = searchItemReceivedStock(lstProjectIds, lstSupplierIds, fromDate, toDate, itemTypeIds)
    ...
    /**
     * method to get a map that contains search-list of Item(s)ReceivedFromSupplier and count
     *
     ...
     * @return -map contains search-list of ItemReceivedStockFromSupplierList and count
     */
    private Map searchItemReceivedStock(List<Long> lstProjectIds, List<Long> lstSupplierIds, Date fromDate, Date toDate, List<Long> lstItemTypeIds) {
        ...
        String queryStr = """SELECT supplier.name AS supplier_name, item.name AS item_name,
        ...
        String queryCount = """
            SELECT COUNT(*) AS count FROM
                (SELECT iit.transaction_entity_id
                FROM inv_inventory_transaction_details iitd
                LEFT JOIN inv_inventory_transaction iit ON iit.id = iitd.inventory_transaction_id
                LEFT JOIN supplier ON supplier.id = iit.transaction_entity_id
                LEFT JOIN item ON item.id = iitd.item_id
                WHERE iit.transaction_type_id=:transactionTypeId
                    ...
                    AND ${queryType} ILIKE :query
                    AND iitd.transaction_date BETWEEN :fromDate AND :toDate
                GROUP BY iit.transaction_entity_id, item.name) as temp
        """
