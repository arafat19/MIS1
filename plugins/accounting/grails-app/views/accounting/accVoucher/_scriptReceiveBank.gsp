...
<script type="text/javascript">
    ...
    function onLoadVoucher() {
        ...
        // update page title
        $(document).attr('title', "MIS - Create Voucher (Receive Bank)");
        loadNumberedMenu(MENU_ID_ACCOUNTING, "#accVoucher/showReceiveBank");
    }
    ...
    function populateOnLoadObjects() {
        ...
        dropDownCoaVoucherType = initKendoDropdown($('#coaVoucherType'), "customName", null, output.lstDebitCoaVoucherType);
        ...
        (
                {
                    ...
                    colModel: [
                        ...
                        {display: "Serial", name: "rowId", width: 20, align: "left", hide: true},
    ...
    function executePreConForDebit() {
        ...
        if (isEmpty($('#hidCoaIdDr').val()) ||
            ...
            showError('Please select an account for debit');
    ...
    function setUrlVoucherGrid() {
        var strUrl = "${createLink(controller:'accVoucher', action: 'listReceiveBank')}";
    ...
    function onChangeCoaVoucherType() {
        ...
        if (coaId != '') {
            ...
            $('#hidCoaCodeDr').val(coaCode);
            $('#hidCoaHeadDr').val(coaHead);
            $('#hidCoaIdDr').val(coaId);
            $('#hidSourceTypeIdDr').val(sourceType);
            ...
            var lstSource = returnObject.sourceList;
            populateSourceCollectionForDr(sourceType, lstSource, '');
            $('#amountDr').focus();
        }
    }
