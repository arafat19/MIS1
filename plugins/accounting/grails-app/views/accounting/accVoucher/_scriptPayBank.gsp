...
<script type="text/javascript">
    ...
    function onLoadVoucher() {
        ...
        // update page title
        $(document).attr('title', "MIS - Create Voucher (Pay Bank)");
        loadNumberedMenu(MENU_ID_ACCOUNTING, "#accVoucher/showPayBank");
    }
    ...
    function setUrlVoucherGrid() {
        var strUrl = "${createLink(controller:'accVoucher', action: 'listPayBank')}";
    ...
    function checkDuplicateCodeInCr(coaId) {
        for (var i = 0; i < gridModelCr.total; i++) {
            if (coaId == parseFloat(gridModelCr.rows[i].cell[0])) {
                showError('Account code ' + gridModelCr.rows[i].cell[1] + ' already exists in debit');
                return false;
            }
        }
    }
