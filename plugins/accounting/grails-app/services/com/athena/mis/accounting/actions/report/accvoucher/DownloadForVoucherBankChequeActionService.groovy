...
/**
 * Download to print voucher(only for bank cheque) in pdf format
 * For details go through Use-Case doc named 'DownloadForVoucherBankChequeActionService'
 */
class DownloadForVoucherBankChequeActionService extends BaseService implements ActionIntf {
    ...
    private static final String JASPER_FILE_NAME = 'bankCheque.jasper'
    private static final String VOUCHER_MAP = "voucherMap"
    private static final String CHEQUE_DATE = "chequeDate"
    private static final String THE_SUM_OF_TAKA_1 = "theSumOfTaka1"
    private static final String THE_SUM_OF_TAKA_2 = "theSumOfTaka2"
    ...
    private static final String RETURN_CHAR = "\r"
    ...
    private Map getVoucherReport(AccVoucher accVoucher, String accountName, boolean isAccountPayable) {
        ...
        Map voucherMap = buildVoucherMap(accVoucher, accountName, isAccountPayable, reportDir)
     ...
     */
    private LinkedHashMap buildVoucherMap(AccVoucher accVoucher, String accountName, boolean isAccountPayable, String reportDir) {
        ...
        voucherMap.put(CHEQUE_DATE, d[0] + (SP * 4) + d[1] + SP + Tools.HYPHEN +
                (SP * 2) + m[0] + (SP * 3) + m[1] + SP + Tools.HYPHEN +
                (SP * 2) + y[0] + (SP * 3) + y[1] + (SP * 4) + y[2] + (SP * 4) + y[3])

        //String inWord = "This is line.This is line.This is lin e123 4523 612723 93 and some other text"     // upto 4523= 47
        String inWord = N2W.convert(accVoucher.amount)
        String inWordWrapped = WordUtils.wrap(inWord, 47)            //wrap text upto max of 55 char
        int idx = inWordWrapped.indexOf(NEW_LINE)
        String inWordLine1 = Tools.EMPTY_SPACE                      // Default value
        String inWordLine2 = Tools.EMPTY_SPACE                      // Default value

        if (idx > 0) {
            inWordLine1 = inWordWrapped.substring(0, idx)
            inWordLine2 = inWordWrapped.substring(idx + 1, inWordWrapped.length())
            inWordLine2 = inWordLine2.replace(Tools.NEW_LINE, Tools.SINGLE_SPACE).replace(NEW_LINE, Tools.SINGLE_SPACE);
        } else {   // string was not wrapped
            inWordLine1 = inWordWrapped
        }

        voucherMap.put(THE_SUM_OF_TAKA_1, (SP * 31) + inWordLine1)
        voucherMap.put(THE_SUM_OF_TAKA_2, inWordLine2)
