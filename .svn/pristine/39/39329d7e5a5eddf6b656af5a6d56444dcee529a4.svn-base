...
class GetDropDownAppUserTagLibActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            List<AppUser> lstAppUser = appUserCacheUtility.enableUserList()
            List lstForDropDown = buildListForDropDown(lstAppUser)
     ...
     */
    private List buildListForDropDown(List<AppUser> lstAppUser) {
        List lstDropDown = []
        if ((lstAppUser == null) || (lstAppUser.size() <= 0))
            return lstDropDown
        for (int i = 0; i < lstAppUser.size(); i++) {
            AppUser user = lstAppUser[i]
            lstDropDown << [id: user.id, name: user.username + Tools.PARENTHESIS_START + user.id + Tools.PARENTHESIS_END]
        }
        return lstDropDown
    }
