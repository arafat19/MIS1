...
class SendRefundTaskToSarbActionService extends BaseService implements ActionIntf {
    ...
    private static final String BOP_305 = '305'
    private static final String BOP_416 = '416'
    ...
    private void buildXml(SarbTaskModel taskModel, SarbCustomerModel custModel) {
        ...
        String originationBank = null
        String originatingCountry = null
        ...
        if (taskModel.paymentMethod == payMethodBankDeposit.id) {
            ...
            originationBank = taskModel.beneficiaryBank
            originatingCountry = taskModel.beneficiaryCountryCode
        }
        int bopCategory = 0
        if (taskModel.remittancePurposeCode.equals(BOP_305)) {
            bopCategory = 300
        } else if (taskModel.remittancePurposeCode.equals(BOP_416)) {
            bopCategory = 400
        }
        Object exhParentTask = exchangeHouseImplService.readTask(taskModel.refundTaskId)
                ...
                'sarbdex:SARBDEXEnvelope'('xmlns:sarbdex': 'x-schema:http://sarbdex.resbank.co.za/schemas/sarbdex_schema.xml') {
                    ...
                    'sarbdex:SARBDEXBody'() {
                        FINSURV('xmlns:xsi': 'http://www.w3.org/2001/XMLSchema-instance', 'xsi:noNamespaceSchemaLocation': 'http://www.w3.org/2001/XMLSchema-instance',
                            ...
                            OriginalTransaction(LineNumber: 1, ReportingQualifier: BOPCUS, Flow: IN, ReplacementTransaction: N, Date: strDate,
                                    TrnReference: taskModel.refNo, BranchCode: branchCode, BranchName: SOUTHEAST, OriginatingBank: originationBank,
                                    OriginatingCountry: originatingCountry, TotalValue: totalValue, ReceivingBank: SOUTHEAST, ReceivingCountry: ZA) {
                                NonResident() {
                                ...
                                MonetaryDetails(SequenceNumber: 1, MoneyTransferAgentIndicator: SOUTHEAST, RandValue: taskModel.amountInLocalCurrency.round(2), ForeignValue: taskModel.amountInForeignCurrency.round(2),
                                        ForeignCurrencyCode: CURRENCY_BDT, BoPCategory: bopCategory, RulingsSection: Tools.EMPTY_SPACE, LocationCountry: taskModel.beneficiaryCountryCode,
                                        ReversalTrnRefNumber: exhParentTask.refNo, ReversalSequence: 1) {
