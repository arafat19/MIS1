package com.athena.mis.accounting.actions.taglib
...
/*Renders html of 'select' for AccGroup objects */
class GetDropDownAccGroupTagLibActionService extends BaseService implements ActionIntf {
    ...
    public Object executePreCondition(Object parameters, Object obj) {
        ...
        try {
            ...
            if (attrs.elements) {
                String elements = attrs.elements
                List lstElementsStr = elements.split(Tools.COMA)
                List lstElements = []
                lstElementsStr.each { lstElements << it }
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            List lstElements = (List) dropDownAttributes.get(ELEMENTS)
            List lstAccGroup = accGroupCacheUtility.listByIsActive()
            String html
            if (lstElements.size() == 0) {
                html = buildDropDown(lstAccGroup, dropDownAttributes)
            } else {
                List lstAccGroupFiltered = []
                for (int i = 0; i < lstAccGroup.size(); i++) {
                    String accGroup = lstAccGroup[i].systemAccGroup
                    for (int j = 0; j < lstElements.size(); j++) {
                        String element = lstElements[j].trim()
                        if (accGroup.equals(element)) {
                            lstAccGroupFiltered << lstAccGroup[i]
                        }
                    }
                }
                html = buildDropDown(lstAccGroupFiltered, dropDownAttributes)
            }
            return html
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstAccGroup - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<AccGroup> lstAccGroup, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstAccGroup = Tools.listForKendoDropdown(lstAccGroup, null, hintsText)  // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstAccGroup as JSON
