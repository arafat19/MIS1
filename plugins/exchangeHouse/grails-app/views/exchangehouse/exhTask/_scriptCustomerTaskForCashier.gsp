<script type="text/javascript">
    ...
    function initVariables() {
        ...
        currentDate = null;
        statusUnApprovedTask = modelJsonForCreateTask.unApprovedTask;
        statusNewTask = modelJsonForCreateTask.statusNewTask;
    }
	...
	function onLoadCreateTask() {
		...
		loadNumberedMenu(MENU_ID_EXCHANGE_HOUSE, "#exhTask/showCustomerTaskForCashier");
	}
    ...
    function populateReadOnlyFields(beneficiaryInstance) {
        ...
        // Labels
        $('#lblCustomerId').text(beneficiaryInstance.customerId);
        ...
        (
                {
                    ...
                    colModel: [
                        ...
                        {display: "Ref No", name: "refNo", width: 100, sortable: false, align: "right"},
                        {display: "Current Status", name: "currentStatus", width: 100, align: "right", hide: true},
                        {display: "Amount(BDT)", name: "amountInForeignCurrency", width: 90, sortable: false, align: "right"},
                        {display: "Amount(" + systemCurrencyName + ")", name: "amountInLocalCurrency", width: 90, sortable: false, align: "right"},
                        {display: "Total Due", name: "total_due", width: 90, sortable: false, align: "right"},
                        {display: "Customer Name", name: "customerName", width: 150, sortable: false, align: "left"},
                        ...
                        {display: "Status", name: "currentStatus", width: 100, sortable: true, align: "left"}
                    ],
                    buttons: [
                    ...
                    searchitems: [
                        {display: "Ref No", name: "ref_no", width: 100, sortable: true, align: "left"},
                        {display: "Customer Name", name: "customer_name", width: 180, sortable: true, align: "left"},
                        {display: "Amount(BDT)", name: "amount_in_foreign_currency", width: 120, sortable: true, align: "left"}
                    ],
                    sortname: "currentStatus",
                    sortorder: "desc",
                    usepager: true,
                    singleSelect: true,
                    title: 'All Tasks',
	...
	function doEdit(com, grid) {
		...
		if (!executeCommonPreConditionForSelect($('#flex1'), 'task', true)) {
			return false;
		}
    ...
    function executePreConditionForApproveTask(ids) {
        var taskStatus = $(ids[ids.length - 1]).find("td").eq(3).find("div").html();
        taskStatus = parseInt(taskStatus);
        switch (taskStatus) {
            case statusNewTask:
                showInfo("Task is already approved.");
                return false;
                break;
        }
        return true;
    ...
    function executePostConditionForApproveTask(data) {
        if(data.isError){
            showError(data.message);
            return false;
        }
        updateListModel(taskGridModel, data.entity, 0);
        $("#flex1").flexAddData(taskGridModel);
        showSuccess(data.message);
        onCompleteAjaxCall();
    }

    function approveTask(com, grid) {
        var ids = $('.trSelected', grid);
        if (!executeCommonPreConditionForSelect($('#flex1'), 'task', true)) {
            return false;
        }
        var taskId = $(ids[ids.length - 1]).attr('id').replace('row', '');
        if (!executePreConditionForApproveTask(ids)) {
            return false;
        }
        showLoadingSpinner(true);
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: "${createLink(controller: 'exhTask',action: 'approveTaskForCashier')}?id=" + taskId,
            success: function (data, textStatus) {
                executePostConditionForApproveTask(data);
            },
            complete: function (XMLHttpRequest, textStatus) {
                onCompleteAjaxCall();
            }
    ...
    function loadFlexGrid() {
        var strUrl = "${createLink(controller: 'exhTask', action: 'listCustomerTaskForCashier')}";
