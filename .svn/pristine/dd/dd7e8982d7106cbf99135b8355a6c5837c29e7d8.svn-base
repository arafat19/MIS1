package com.athena.mis.qs.actions.taglib
...
class GetDropDownQsSiteTagLibActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            String html = buildDropDown(lstSiteObj, dropDownAttributes)
            return html
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstSite - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List lstSite, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstSite = Tools.listForKendoDropdown(lstSite, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstSite as JSON
