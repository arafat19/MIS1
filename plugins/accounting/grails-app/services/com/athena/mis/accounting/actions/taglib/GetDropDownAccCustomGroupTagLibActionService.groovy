...
/*Renders html of 'select' for AccCustomGroup objects */
class GetDropDownAccCustomGroupTagLibActionService extends BaseService implements ActionIntf {
    ...
    /** Get the list of AccCustomGroups
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            List lstAccCustomGroup = accCustomGroupCacheUtility.listByIsActive()
            String html = buildDropDown(lstAccCustomGroup, dropDownAttributes)
            return html
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @param lstAccCustomGroup - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<AccCustomGroup> lstAccCustomGroup, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstAccCustomGroup = Tools.listForKendoDropdown(lstAccCustomGroup, null, hintsText)  // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstAccCustomGroup as JSON
