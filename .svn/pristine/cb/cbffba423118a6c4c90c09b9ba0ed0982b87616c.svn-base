...
/**
 *  Class to get search result of inventory-consumption(parent) for grid
 *  For details go through Use-Case doc named 'SearchForInventoryConsumptionActionService'
 */
class SearchForInventoryConsumptionActionService extends BaseService implements ActionIntf {
    ...
    /**
     *  wrap search list of inventory-consumption(parent) for grid
     *
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            if (inventoryIdList.size() > 0) { //if userMappedInventoryIds > 0 then get list of inventory-consumption(parent) of that Inventories
                LinkedHashMap serviceReturn = (LinkedHashMap) searchForInventoryConsumption(inventoryIdList)
                ...
                inventoryConsumptionListWrap = (List) wrapInventoryTransactionList(inventoryConsumptionList, this.start)
            }
    ...
    /**
     * Get search result of ConsumptionList(parent)
     ...
     */
    private LinkedHashMap searchForInventoryConsumption(List<Long> inventoryIdList) {
        ...
        String queryStr = """
          SELECT iit.id, to_char(iit.transaction_date, 'dd-Mon-YYYY') AS transaction_date_str,
            ...
            WHERE iit.inventory_id IN (${inventoryIds}) AND
                  iit.transaction_type_id = ${transactionTypeConsumption.id} AND
                  iit.transaction_entity_type_id = ${transactionEntityNone.id} AND
                  iit.budget_id > 0
