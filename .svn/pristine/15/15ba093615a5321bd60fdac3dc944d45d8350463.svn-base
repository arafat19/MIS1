...
/**
 *  Show pay cash voucher in the grid
 *  For details go through Use-Case doc named 'ShowAccVoucherPayCashActionService'
 */
class ShowAccVoucherPayCashActionService extends BaseService implements ActionIntf {
    ...
    private static final String LST_CREDIT_COA_VOUCHER_TYPE = "lstCreditCoaVoucherType"
    ...
    /**
     * Get  pay cash voucher list for grid
    ...
    /**
     * Wrap pay cash voucher list for grid
    ...
    public Object buildSuccessResultForUI(Object obj) {
        Map result = new LinkedHashMap()
        try {
            Map executeResult = (Map) obj          // cast object received from execute method
            ...
            long companyId = accSessionUtil.appSessionUtil.getCompanyId()
            SystemEntity voucherTypeObj = (SystemEntity) accVoucherTypeCacheUtility.readByReservedAndCompany(accVoucherTypeCacheUtility.PAYMENT_VOUCHER_CASH_ID, companyId)
            List lstCoaVoucherType = accVoucherTypeCoaCacheUtility.listCoa(voucherTypeObj)
            lstCoaVoucherType = Tools.listForKendoDropdown(lstCoaVoucherType, CUSTOM_NAME, null)
            result.put(LST_CREDIT_COA_VOUCHER_TYPE, lstCoaVoucherType)
            result.put(Tools.IS_ERROR, Boolean.FALSE)
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
    ...
    /**
     * Wrap list of pay cash voucher in grid entity
     * @param voucherList -list of pay cash voucher object(s)
     * @param start -starting index of the page
     * @return -list of wrapped pay cash voucher
     */
