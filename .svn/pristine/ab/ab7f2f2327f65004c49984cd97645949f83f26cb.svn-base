...
/**
 *  Show UI for accIouSlip CRUD and list of accIouSlip for grid
 *  For details go through Use-Case doc named 'ShowAccIouSlipActionService'
 */
class ShowAccIouSlipActionService extends BaseService implements ActionIntf {

    private static final String DEFAULT_ERROR_MESSAGE = "Failed to load account IOU slip page"
    ...
    /**
     * Get indent list by using projectId, employee list, project list and get indent list for dropDown
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            initPager(params)    // initialize parameters for grid
            ...
            LinkedHashMap serviceReturn = list(projectIdList) // Get accIouList by projectIdList using list() method
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            ...
            List accIouSlipList = (List) executeResult.get(ACC_IOU_SLIP_LIST)
            int count = (int) executeResult.get(COUNT)
            List resultList = wrapAccIouSlipListInGridEntity(accIouSlipList, start)
            Map output = [page: pageNumber, total: count, rows: resultList]

            result.put(ACC_IOU_SLIP_LIST, output)
            result.put(Tools.IS_ERROR, Boolean.FALSE)
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
     ...
     */
    private List wrapAccIouSlipListInGridEntity(List<GroovyRowResult> accIouSlipList, int start) {
