...
class ListSubCategoryFavouriteActionService extends BaseService implements ActionIntf{
     ...
     */
    private List<GridEntity> wrapDistributionPoint(List<GroovyRowResult> lstDistributionPoint, int start) {
...
//                        "${favouritesRowResult.id}",
    ...
    private Map subCategoryFavouriteList() {
        ...
        String QUERY_FOR_COUNT = """
            SELECT COUNT(DISTINCT(docscum.id)) count
            FROM doc_sub_category_user_mapping docscum
            LEFT JOIN doc_category dc ON dc.id = docscum.category_id
            LEFT JOIN doc_sub_category dsc ON dsc.id = docscum.sub_category_id
            WHERE docscum.company_id =:companyId
        ...
        Map queryParams = [companyId: appUser.companyId, userId: appUser.id]
