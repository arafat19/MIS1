<script type="text/javascript">
    var taskListModel, dropDownExchangeHouse, dropDownCurrency, dropDownPaymentMethod;
    var payMethodBankDepositId, payMethodCashCollectionId, currentDate, numericAmountModel, numericAmountInLocalCurrencyModel;
    ...
    function onLoadTask() {
        ...
        loadNumberedMenu(MENU_ID_ARMS, "#rmsTask/show");
    }

    // method call on change of ExchangeHouse dropdown
    function onChangeExchangeHouse() {
        if (dropDownExchangeHouse.value() == '') {
            $("#spanCountry").text('N/A');
            return;
        }
        $("#spanCountry").text(dropDownExchangeHouse.dataItem().country);
        $("#countryId").val(dropDownExchangeHouse.dataItem().countryId);
    }
    ...
    function clearTaskForm() {
        clearForm($("#rmsTaskBasicInfoForm"), dropDownPaymentMethod);  // clear errors & form values
        onChangeExchangeHouse();
    ...
    function selectRmsTask(com, grid) {
        ...
        $.ajax({
            url: "${createLink(controller:'rmsTask', action: 'select')}?id=" + id,
    ...
    function showRmsTask(data) {
        ...
        disableProperty(data);
    }

    function disableProperty(data) {
        if (data.disableExh) {
            dropDownExchangeHouse.readonly();
        }
        if (data.disablePay) {
            dropDownPaymentMethod.readonly();
        }
    }
    ...
    function deleteRmsTask(com, grid) {
        ...
        $.ajax({
            url: "${createLink(controller:'rmsTask', action:  'delete')}?id=" + rmsTaskId,
    ...
    function populateFlex() {
        var strUrl = "${createLink(controller:'rmsTask',action:  'list')}";
