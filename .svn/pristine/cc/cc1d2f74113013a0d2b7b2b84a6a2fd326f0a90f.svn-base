<script type="text/javascript">
    ...
    $(document).ready(function () {
        ...
        $('#printTrialBalancePdf').click(function () {
            printTrialBalanceListOfLevel3('pdf');
            return false;
        });
        $('#printTrialBalanceCsv').click(function () {
            printTrialBalanceListCsvOfLevel3();
            return false;
        });
        onLoadTrialBalanceListOfLevel3();
    });

    function printTrialBalanceListOfLevel3(formatType) {
        var confirmMsg;
        if (formatType == 'pdf') {
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadTrialBalanceOfLevel3')}" + params;
    ...
    function printTrialBalanceListCsvOfLevel3() {
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadTrialBalanceCsvOfLevel3')}" + params;
            document.location = url;
        }
        showLoadingSpinner(false);
        return true
    ...
    function onLoadTrialBalanceListOfLevel3() {
        try {   // Model will be supplied to grid on load _list.gsp
            initializeForm($("#searchForm"), getTrialBalanceListOfLevel3);
        ...
        loadNumberedMenu(MENU_ID_ACCOUNTING, "#accReport/showTrialBalanceOfLevel3");
    }
    ...
    function getTrialBalanceListOfLevel3() {
        ...
        showLoadingSpinner(true);
        $.ajax({
            url: "${createLink(controller:'accReport', action: 'listTrialBalanceOfLevel3')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId,
    ...
    function populateTrialBalanceListGrid(data) {
        ...
        var trialBalanceListUrl = "${createLink(controller:'accReport', action: 'listTrialBalanceOfLevel3')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId;
    ...
    (
            {
                ...
                usepager: false,
                singleSelect: true,
                title: 'Trial Balance (Hierarchy 3)',
                useRp: true,
                rp: 20,
