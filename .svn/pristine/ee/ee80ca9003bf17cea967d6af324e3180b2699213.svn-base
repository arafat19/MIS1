...
/**
 *  Show list of disbursed or paid task for grid for Customer
 *  For details go through Use-Case doc named 'ExhListDisbursedTaskForCustomerActionService'
 */
class ExhListDisbursedTaskForCustomerActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            LinkedHashMap serviceReturn = listDisbursedTask()      // get list of disbursed task
    ...
    /**
     * Get list & count of task which status is SENT_TO_BANK, SENT_TO_OTHER_BANK,RESOLVED_BY_OTHER_BANK
     */
    private LinkedHashMap listDisbursedTask() {
        ...
        List taskStatus = [exhSentToBankSysEntityObject.id,
                exhSentToOtherBankSysEntityObject.id,
                exhResolvedByOtherBankSysEntityObject.id]
        List<ExhTask> taskList = ExhTask.findAllByCompanyIdAndCustomerIdAndCurrentStatusInList(
                companyId,
                customerId,
                taskStatus,
                [max: resultPerPage, offset: start, sort: sortColumn, order: sortOrder, readOnly: true]
        )
        int count = ExhTask.countByCompanyIdAndCustomerIdAndCurrentStatusInList(
                companyId,
                customerId,
                taskStatus
        )
