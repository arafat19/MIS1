<script type="text/javascript">
    var modelJsonForIncomeStatementList, dropDownProject, dropDownDivision;
    jQuery(function ($) {
        modelJsonForIncomeStatementList = ${modelJson};
        $('#printIncomeStatementPdf').click(function () {
            printIncomeStatementListOfLevel4('pdf');
            return false;
        });

        $('#printIncomeStatementCsv').click(function () {
            printIncomeStatementListCsvOfLevel4();
            return false;
        });
        onLoadIncomeStatementListOfLevel4();
    });

    function printIncomeStatementListOfLevel4(formatType) {
        var confirmMsg;
        if (formatType == 'pdf') {
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadIncomeStatementOfLevel4')}" + params;
    ...
    function printIncomeStatementListCsvOfLevel4() {
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadIncomeStatementCsvOfLevel4')}" + params;
            document.location = url;
        }
        showLoadingSpinner(false);
        return true
    ...
    function onLoadIncomeStatementListOfLevel4() {
        try {   // Model will be supplied to grid on load _list.gsp
            $('.download_icon_set').hide();
            initializeForm($("#searchForm"), getIncomeStatementListOfLevel4);
        ...
        loadNumberedMenu(MENU_ID_ACCOUNTING, "#accReport/showIncomeStatementOfLevel4");
    }

    function getIncomeStatementListOfLevel4() {
        ...
        showLoadingSpinner(true);
        $.ajax({
            url: "${createLink(controller:'accReport', action: 'listIncomeStatementOfLevel4')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId,
    ...
    function populateIncomeStatementListGrid(data) {
        ...
        var trialBalanceListUrl = "${createLink(controller:'accReport', action: 'listIncomeStatementOfLevel4')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId;
    ...
    (
            {
                ...
                usepager: false,
                singleSelect: true,
                title: 'Income Statement (Hierarchy 4)',
                useRp: true,
                rp: 20,
