...
/**
 *  Class to get list of accIpc object(s) to show on grid
 *  For details go through Use-Case doc named 'ListAccIpcActionService'
 */
class ListAccIpcActionService extends BaseService implements ActionIntf {
    ...
    private static final String PAGE_LOAD_ERROR_MESSAGE = "Failed to load IPC grid"
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            LinkedHashMap accIpc = list(projectIdList)
            List<GroovyRowResult> accIpcList = accIpc.listOfAccIpc
    ...
    /**
     * get list of accIpc object by user mapped projectIds
     * @param lstProjectIds -Project.id
     * @return -a map contains accIpc objects and count
     */
    private LinkedHashMap list(List<Long> lstProjectIds) {
        ...
        String queryStr = """
               SELECT ipc.id id, ipc.ipc_no ipc_no, p.name project_name
                    FROM acc_ipc ipc
                     ...
                     WHERE ipc.project_id IN (${strProjectIds})
        ...
        return [listOfAccIpc: result, count: total]
    }
}
