...
/**
 * Download  financial statement in csv format for level 2
 * Report will contain coa of tier1(if any) and sum of tier2
 * For details go through Use-Case doc named 'DownloadForFinancialStatementCsvOfLevel2ActionService'
 */
class DownloadForFinancialStatementCsvOfLevel2ActionService extends BaseService implements ActionIntf {
    ...
    private static final String ACC_BALANCE_SHEET_REPORT_CSV = 'accFinancialStatementOfLevel2Csv.jasper'
    ...
    private Map getFinancialStatementReport(Date fromDate, Date toDate, List<Long> projectIdList, String projectName, long postedByParam, long divisionId, String divisionName) {
        ...
        String reportDir = Tools.getAccountingReportDirectory() + File.separator + REPORT_FOLDER + File.separator + LEVEL2
    ...
    private List<GroovyRowResult> getFinancialStatementList(Date fromDate, Date toDate, List<Long> projectIds, long postedByParam, long divisionId) {
        ...
        String queryStr = """
            ...
            GROUP BY coa.acc_type_id, t1.name, t1.id, type.order_id
