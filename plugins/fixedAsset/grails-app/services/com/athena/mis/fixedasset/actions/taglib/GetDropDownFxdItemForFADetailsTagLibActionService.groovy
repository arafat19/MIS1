...
/*Renders html of 'select' for Fixed asset items objects */

class GetDropDownFxdItemForFADetailsTagLibActionService extends BaseService implements ActionIntf {
    ...
    /** Get the list of fixed asset items
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            List lstItem = listOfFixedAssetItem()
    ...
    private List<GroovyRowResult> listOfFixedAssetItem() {
        long companyId = fxdSessionUtil.appSessionUtil.getCompanyId()
        SystemEntity categoryTypeFixedAsset = (SystemEntity) itemCategoryCacheUtility.readByReservedAndCompany(itemCategoryCacheUtility.FIXED_ASSET, companyId)
        Map queryParams = [
                itemCategoryId: categoryTypeFixedAsset.id,
                companyId: companyId
        ]
    ...
    private String buildDropDown(List lstItem, Map dropDownAttributes) {
        ...
        if (showHints.booleanValue()) {
            lstItem = Tools.listForKendoDropdown(lstItem, null, hintsText)  // the null parameter indicates that dataTextField is name
        }
