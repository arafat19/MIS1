...
/**
 *  Show list of financial statement of level 5 (show coa of all(tier1, tier2, tier3, tier4, tier5(if any)) for grid
 *  For details go through Use-Case doc named 'ListForFinancialStatementOfLevel5ActionService'
 */
class ListForFinancialStatementOfLevel5ActionService extends BaseService implements ActionIntf {

    private Logger log = Logger.getLogger(getClass())

    private static final String FINANCIAL_STATEMENT_NOT_FOUND = "Financial statement not found within given dates"
     ...
     * @param parameters - serialized parameters from UI
     * @param obj - N/A
     * @return -a map containing all financial statement list for level 5 and isError(true/false)
     */
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            if (returnFinancialStatementList.size() <= 0) {
                result.put(Tools.MESSAGE, FINANCIAL_STATEMENT_NOT_FOUND)
                return result
            }
    ...
    /**
     * Get list of financial statement of level 5 for showing in the grid
    ...
    /**
     * Wrap list of income statement of level 5 in grid entity
    ...
    /**
     * Calculate financial statement balance of level 5 - sum of Debit(Asset) and sum of Credit(Liability)
    ...
    private List<GroovyRowResult> getFinancialStatementList(Date fromDate, Date toDate, List<Long> projectIds, long postedByParam, long divisionId) {
        String lstProjectIds = Tools.buildCommaSeparatedStringOfIds(projectIds)
        ...
        if (divisionId > 0) {
            ...
            subQueryProject = " AND details.project_id IN (${lstProjectIds}) "
        }
        String queryStr = """
     ...
     */
    private Map getIncomeStatementSum(Date fromDate, Date toDate, List<Long> projectIds, long postedByParam, long divisionId) {
        String lstProjectIds = Tools.buildCommaSeparatedStringOfIds(projectIds)
        long companyId = accSessionUtil.appSessionUtil.getCompanyId()
