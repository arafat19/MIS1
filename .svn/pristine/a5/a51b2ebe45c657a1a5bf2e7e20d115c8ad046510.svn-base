...
/**
 *  Show list of inventory transaction(Inventory In From Inventory) for grid
 *  For details go through Use-Case doc named 'ListForInventoryInFromInventoryActionService'
 */
class ListForInventoryInFromInventoryActionService extends BaseService implements ActionIntf {
    ...
    private static final String SERVER_ERROR_MESSAGE = "Could not get Inventory-In transaction List"
    ...
    /**
     * Get inventory transaction list (Inventory In From Inventory) for grid
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            if (inventoryIds.size() > 0) {
                ...
                lstInventoryIn = (List<GroovyRowResult>) serviceReturn.inventoryInList
    ...
    private LinkedHashMap listForInventoryInFromInventory(List<Long> inventoryIdList) {
       ...
       FROM inv_inventory_transaction iit
          ...
          WHERE iit.inventory_id IN (${inventoryIds})
                AND iit.transaction_type_id=:transactionTypeIdIn
