...
class SarbTaskDetailsService extends BaseService {
    ...
    private static final String UPDATE_FOR_MOVE = """
        UPDATE sarb_task_details SET
            version = version+1,
    ...
    public int updateForMove(SarbTaskDetails sarbTaskDetails, long reviseStatusId) {
        Map queryParams = [
                id: sarbTaskDetails.id,
                version: sarbTaskDetails.version,
                reviseStatus: reviseStatusId
        ]
        int updateCount = executeUpdateSql(UPDATE_FOR_MOVE, queryParams)
        if (updateCount <= 0) {
            throw new RuntimeException("Failed to update SarbTaskDetails")
        }
        sarbTaskDetails.version = sarbTaskDetails.version + 1
