<script type="text/javascript">
    ...
    function onLoadCreateTask() {
	    ...
	    initGridTaskForAgent();
        loadFlexGrid();

        // update page title
        $(document).attr('title', "ARMS - Create Task");
        loadNumberedMenu(MENU_ID_EXCHANGE_HOUSE, "#exhTask/showForAgent");
    }
	...
	function executePostCondition(result, isNew) {
		if (result.isError == true) {
			if (result.isConfirmationIssues && result.lstErrors != null) {
				...
				//$('#taskCreateConfirmationDialog').dialog('open');
    ...
    function executePreConditionForSentToExchangeHouse(selectedIds) { //
        if (selectedIds.length == 0) {
            showError("Please select a Task to send");
            return false;
        }

        if (!confirm('Are you sure you want to send ' + selectedIds.length + ' task(s) to Exchange House?')) {
            return false;
        }
        return true;
    ...
    function executePostConditionForSentToExchangeHouse(data, ids) {
        if (data.isError) {
            showError(data.message);
            return false;
        }
        if (!data.isConfirmationIssue) {
            refreshGridAfterTaskSend(data);
            return false;
        }
        if (!confirm(data.message)) return false;
        sendTaskAfterConfirmation(ids);
    }

    function sendTaskAfterConfirmation(ids) {
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: "${createLink(controller: 'exhTask',action: 'sendToExchangeHouse')}?ids=" + ids + "&isConfirmed=true",
            success: function (data, textStatus) {
                refreshGridAfterTaskSend(data);
            },
            complete: function (XMLHttpRequest, textStatus) {
    ...
    function sendTaskToExchangeHouse(com, grid) {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: "${createLink(controller: 'exhTask',action: 'sendToExchangeHouse')}?ids=" + ids + "&isConfirmed=false",
            success: function (data, textStatus) {
                executePostConditionForSentToExchangeHouse(data, ids);
            },
            complete: function (XMLHttpRequest, textStatus) {
                $('span.send').show();
                onCompleteAjaxCall();
    ...
    function loadFlexGrid() {
        ...
        if (beneficiaryId > 0) {
            strUrl = "${createLink(controller: 'exhTask', action: 'listForAgent')}?beneficiaryId=" + beneficiaryId;
        } else {
            strUrl = "${createLink(controller: 'exhTask', action: 'listForAgent')}";
        }
