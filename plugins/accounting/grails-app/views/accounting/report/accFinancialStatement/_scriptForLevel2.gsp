<script type="text/javascript">
    var modelJsonForFinancialStatementList, validatorFinanStatementL2, dropDownProject, dropDownDivision;
    jQuery(function ($) {
        modelJsonForFinancialStatementList = ${modelJson};
        $('#printFinancialStatementPdf').click(function () {
            printFinancialStatementListOfLevel2('pdf');
            return false;
        });

        $('#printFinancialStatementCsv').click(function () {
            printFinancialStatementCsvOfLevel2();
            return false;
        });
        onLoadFinancialStatementOfLevel2();
    });

    function printFinancialStatementListOfLevel2(formatType) {
        var confirmMsg;
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadFinancialStatementOfLevel2')}" + params;
    ...
    function printFinancialStatementCsvOfLevel2() {
        ...
        if (confirm(confirmMsg)) {
            var url = "${createLink(controller:'accReport', action: 'downloadFinancialStatementCsvOfLevel2')}" + params;
            document.location = url;
        }
        showLoadingSpinner(false);
        return true
    ...
    function onLoadFinancialStatementOfLevel2() {
        try {   // Model will be supplied to grid on load _list.gsp
            $('.download_icon_set').hide();
            initializeForm($("#searchForm"), getFinancialStatementListOfLevel2);
        ...
        loadNumberedMenu(MENU_ID_ACCOUNTING, "#accReport/showFinancialStatementOfLevel2");
    }

    function getFinancialStatementListOfLevel2() {
        ...
        showLoadingSpinner(true);
        $.ajax({
            url: "${createLink(controller:'accReport', action: 'listFinancialStatementOfLevel2')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId,
    ...
    function populateFinancialStatementListGrid(data) {
        ...
        var financialStatementListUrl = "${createLink(controller:'accReport', action: 'listFinancialStatementOfLevel2')}?fromDate=" + fromDate + "&toDate=" + toDate + "&projectId=" + projectId + "&divisionId=" + divisionId;
        ...
        (
                {
                    ...
                    colModel: [
                        ...
                        {display: "Tier1 ID", name: "coaId", width: 30, sortable: false, align: "right", hide: "true"},
                    ...
                    usepager: false,
                    singleSelect: true,
                    title: 'Financial Statement (Hierarchy-2)',
                    useRp: true,
                    rp: 20,
