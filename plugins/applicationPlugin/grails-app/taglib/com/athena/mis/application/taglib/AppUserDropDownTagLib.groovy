...
class AppUserDropDownTagLib {
    ...
    def dropDownAppUser = { attrs, body ->
        Map preResult = (Map) getDropDownAppUserTagLibActionService.executePreCondition(attrs, null)
        Boolean isError = preResult.isError
        if (isError.booleanValue()) {
            out << Tools.EMPTY_SPACE
            return
        }
        String html = getDropDownAppUserTagLibActionService.execute(preResult, null)
        out << html
    }
}
