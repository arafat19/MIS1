...
class RetrieveAgainSarbTaskResponseActionService extends BaseService implements ActionIntf {
    ...
    private static final String ALREADY_ACCEPTED = "Task already accepted by SARB"
    ...
    public Object executePreCondition(Object parameters, Object obj) {
        ...
        try {
            ...
            if (sarbTaskModel.isAcceptedBySarb) {
                result.put(Tools.MESSAGE, ALREADY_ACCEPTED)
                return result
            }
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            result.put(Tools.IS_ERROR, Boolean.FALSE)
            result.put(Tools.ENTITY, taskModel)
            return result
        }
        catch (Exception ex) {
            log.error(ex.getMessage())
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            Map executeResult = (Map) obj
            SarbTaskModel taskModel = (SarbTaskModel) executeResult.get(Tools.ENTITY)
            SystemEntity paymentMethodObj = exchangeHouseImplService.readExhPaymentMethod(taskModel.paymentMethod)
            GridEntity gridObj = new GridEntity()
            gridObj.id = taskModel.id
            gridObj.cell = [
                    Tools.LABEL_NEW,
                    taskModel.id,
                    taskModel.refNo,
                    taskModel.customerName,
                    taskModel.beneficiaryName,
                    paymentMethodObj ? paymentMethodObj.key : Tools.EMPTY_SPACE,
                    taskModel.submittedFileCount,
                    taskModel.amountInForeignCurrency,
                    taskModel.amountInLocalCurrency,
                    taskModel.isAcceptedBySarb ? Tools.YES : LABEL_NO_RED
            ]
            result.put(Tools.ENTITY, gridObj)
            result.put(Tools.IS_ERROR, Boolean.FALSE)
