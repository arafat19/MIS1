...
class GetDropDownExhCurrencyConversionTagLibActionService {
	...
	public Object execute(Object parameters, Object obj) {
		try {
			...
			List<Currency> lstCurrency = buildFromToCurrencyCombo()
			String html = buildDropDown(lstCurrency, dropDownAttributes)
			return html
		} catch (Exception e) {
			log.error(e.message)
	...
	private List buildFromToCurrencyCombo() {
		Currency fromCurrency = (Currency) currencyCacheUtility.getLocalCurrency()
		Currency toCurrency = (Currency) currencyCacheUtility.getForeignCurrency()
	...
	private String buildDropDown(List<Currency> lstCurrency, Map dropDownAttributes) {
		...
		if (showHints.booleanValue()) {
			lstCurrency = Tools.listForKendoDropdown(lstCurrency, LABEL_SYMBOL, hintsText)  // the null parameter indicates that dataTextField is name
		}
