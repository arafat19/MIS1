...
/**
 *  Show UI for inventory transaction(Inventory In From Inventory) CRUD and list of inventory transaction for grid
 *  For details go through Use-Case doc named 'ShowForInventoryInFromInventoryActionService'
 */
class ShowForInventoryInFromInventoryActionService extends BaseService implements ActionIntf {
    ...
    private static final String DEFAULT_ERROR_MESSAGE = "Can't load Inventory In From Inventory page"
    ...
    /**
     * Get inventory type list for dropDown and inventory transaction(Inventory In From Inventory) list for grid
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            if (inventoryIds.size() > 0) {
                serviceReturn = listForInventoryInFromInventory(inventoryIds)
     ...
     */
    private LinkedHashMap listForInventoryInFromInventory(List<Long> inventoryIdList) {
        long companyId = invSessionUtil.appSessionUtil.getCompanyId()
        SystemEntity transactionTypeObj = (SystemEntity) invTransactionTypeCacheUtility.readByReservedAndCompany(invTransactionTypeCacheUtility.TRANSACTION_TYPE_IN, companyId)
        ...
        // query for list
        String queryStr = """
           SELECT iit.id, to_char(iit_out.transaction_date, 'dd-Mon-YYYY') AS transfer_date_str, to_char(iit.transaction_date, 'dd-Mon-YYYY') AS transaction_date_str,
                   ...
                   iit.item_count, COUNT(iitd.id) AS total_approve, iit_out.id AS transfer_id,
        ...
        String queryCount = """
            ...
            WHERE iit.inventory_id IN (${inventoryIds}) AND
                  iit.transaction_type_id=:transactionTypeId
                  AND iit.transaction_entity_type_id=:transactionEntityTypeId
        """
