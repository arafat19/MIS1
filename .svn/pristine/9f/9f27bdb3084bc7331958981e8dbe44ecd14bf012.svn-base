...
/**
 *  Class to get search result list of User-Role mapping object(s) to show on grid
 *  For details go through Use-Case doc named 'SearchUserRoleActionService'
 */
class SearchUserRoleActionService extends BaseService implements ActionIntf {
    ...
    private static final String PAGE_LOAD_ERROR_MESSAGE = "Failed to search User-Role list"
    private static final String ROLE_OBJ = "roleObj"
    private static final String ROLE_NOT_FOUND = "Selected role not found"

    /**
     * check access permission to search list of User-Role mapping object(s)
    ...
    public Object executePreCondition(Object parameters, Object obj) {
        ...
        try {
            ...
            //Only admin has right to search User-Role mapping object(s)
    ...
    /**
     * get search result list of User-Role mapping object(s)
    ...
    private static final String QUERY_FOR_LIST = """
        ...
        AND app_user.company_id = :companyId AND
    ...
    FROM user_role
    LEFT JOIN app_user ON app_user.id = user_role.user_id
    WHERE app_user.username ilike :userName
    AND app_user.enabled = true
    AND app_user.company_id = :companyId
    AND user_role.role_id = :roleId
    """

    /**
     * get search result list of User-Role mapped object(s) to show on grid
     * @return -a map contains list of userRole mapped object(s) and count
     */
    private Map search(Role role) {
        Map queryParams = [
                userName: Tools.PERCENTAGE + query + Tools.PERCENTAGE,
                roleId: role.id,
