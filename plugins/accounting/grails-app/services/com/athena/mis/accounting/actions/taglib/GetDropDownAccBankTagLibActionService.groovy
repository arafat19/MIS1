...
class GetDropDownAccBankTagLibActionService extends BaseService implements ActionIntf {
     ...
     * @param bankList - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List bankList, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            bankList = Tools.listForKendoDropdown(bankList, null, hintsText)  // the null parameter indicates that dataTextField is name
        }
        String jsonData = bankList as JSON
