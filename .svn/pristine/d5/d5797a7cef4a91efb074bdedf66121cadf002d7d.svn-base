<script type="text/javascript">
    ...
    function onLoadCreateTask() {
        ...
        initGridTaskForCustomer();
        loadFlexGrid();

        $(document).attr('title', "ARMS - Create Task");
        loadNumberedMenu(MENU_ID_EXCHANGE_HOUSE, "#exhTask/showForCustomer");
    }
    ...
    function resetBeneficiaryObject(beneficiaryObject) {
        ...
        beneficiaryObject.customerCode = '';
...
//                confirmationErrors = confirmationErrors + "<br />" + "<br />" + 'Note: Total transaction of the customer will be '+localCurrencyName+' ' + totalAmount;
                var amountNote = 'Note: Total transaction of the customer will be ' + systemCurrencyName + ' ' + totalAmount;
                confirmationErrors = confirmationErrors ? (confirmationErrors + "<br />" + "<br />") : '';
                confirmationErrors = confirmationErrors + amountNote;
    ...
    function showBeneficiaryData(paymentMethod, beneficiaryInstance) {
        ...
        switch (paymentMethodId) {
            ...
            case cashCollectionId:
                //enablePinBranchInfo(beneficiaryInstance);
    ...
    function initGridTaskForCustomer() {
        $("#flex1").flexigrid
        (
                {
                    url: false,
                    ...
                    buttons: [
                        ...
                        {name: 'Send To Exchange House', bclass: 'send', onpress: sendTaskToExchangeHouse},
    ...
    function executePreConditionForSendToExchangeHouse(selectedIds) { //
        if (selectedIds.length == 0) {
            showError("Please select a Task to send");
            return false;
        }
        return confirm('Are you sure you want to send ' + selectedIds.length + ' task(s) to Exchange House?');
    }

    function executePostConditionForSendToExchangeHouse(data, ids) {
        if (data.isError) {
            showError(data.message);
            return false;
        }
        refreshGridAfterTaskSend(data);
    ...
    function refreshGridAfterTaskSend(data) {
        var selectedIds = $('.trSelected', $('#flex1'));
        var selectedRow = null;
        $(selectedIds).each(function (e) {
            selectedRow = $(this).remove();
            removeEntityFromGridRows(taskGridModel, selectedRow);
        });
        $('#flex1').decreaseCount(selectedIds.length);
        taskGridModel.total = parseInt(taskGridModel.total) - (selectedIds.length);
        showSuccess(data.message);
    }

    function sendTaskToExchangeHouse(com, grid) {
        ...
        $.ajax({
            type: 'post',
            dataType: 'json',
            url: "${createLink(controller: 'exhTask',action: 'sendToExhForCustomer')}?ids=" + ids + "&isConfirmed=true",
            success: function (data, textStatus) {
                executePostConditionForSendToExchangeHouse(data, ids);
            },
            complete: function (XMLHttpRequest, textStatus) {
                $('span.send').show();
                onCompleteAjaxCall();
    ...
    function loadFlexGrid() {
        var strUrl;
        var beneficiaryId = parseInt(modelJsonForCreateTask.beneficiaryId, 10);
        if (beneficiaryId > 0) {
            strUrl = "${createLink(controller: 'exhTask', action: 'listForCustomer')}?beneficiaryId=" + beneficiaryId;
        } else {
            strUrl = "${createLink(controller: 'exhTask', action: 'listForCustomer')}";
        }
        $("#flex1").flexOptions({url: strUrl});
        if (taskGridModel) {
            $("#flex1").flexAddData(taskGridModel);
//            $('div.mDiv > div.ftitle').text('All Remittance of ' + modelJsonForCreateTask.customerName);
        }
    }
