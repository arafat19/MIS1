...
/**
 *  Reverse adjustment of approved inventory transaction details object (Inventory In From Inventory) and remove form grid
 *  For details go through Use-Case doc named 'ReverseAdjustForInvInFromInventoryActionService'
 */
class ReverseAdjustForInvInFromInventoryActionService extends BaseService implements ActionIntf {
    ...
    /**
     * Checking pre condition before doing reverse adjustment of transaction details object (Inventory In From Inventory)
     ...
     * Reverse related inventory transaction details out object
        -Set isCurrent false of current inventory transaction details out object
        -Decrease item count of inventory transaction out object (parent object)
     * Set isCurrent false of current inventory transaction details in object
     * Decrease item count of inventory transaction in object (parent object)
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            reverseAdjDetails.rate = calculateValuationForOut(currentTransactionDetails.actualQuantity, currentTransactionDetails.inventoryId, currentTransactionDetails.itemId)
            ...
            // when reversing the invInFromInv then its related invOut will be reversed
            InvInventoryTransactionDetails currentTransactionOutDetails = invInventoryTransactionDetailsService.read(currentTransactionDetails.transactionDetailsId)
            // copy current transaction details out object for reverse adjustment
            InvInventoryTransactionDetails reverseInvOutDetails = copyForReverseAdjustment(currentTransactionOutDetails, newReverseAdjDetails.comments, invSessionUtil.appSessionUtil.getAppUser())
            // update necessary properties of current transaction details out object for reverse adjustment (isCurrent = false)
            currentTransactionOutDetails.updatedBy = invSessionUtil.appSessionUtil.getAppUser().id
            currentTransactionOutDetails.updatedOn = new Date()
            currentTransactionOutDetails.isCurrent = false
            // reverse adjustment for invOutDetails
            if (reverseInvOutDetails) {
                // create reverse inventory transaction details out object (isCurrent = false)
                invInventoryTransactionDetailsService.create(reverseInvOutDetails)
                // set isCurrent false of current inventory transaction details out object
                setIsCurrentTransaction(currentTransactionOutDetails)
                // decrease item count of inventory transaction out object (parent object)
                decreaseItemCount(currentTransactionOutDetails.inventoryTransactionId)
            }
            // set isCurrent false of current inventory transaction details in object
