...
class GetDropDownExhCashierTagLibActionService {
	...
	public Object execute(Object parameters, Object obj) {
		try {
			...
			List<AppUser> validCashierList = listCashier(companyId)
			String html = buildDropDown(validCashierList, dropDownAttributes)
			return html
		} catch (Exception e) {
			log.error(e.message)
	...
	private List<AppUser> listCashier(Long companyId) {

		List userIds = UserRole.withCriteria {
			eq('role.id', roleTypeCacheUtility.ROLE_TYPE_EXH_CASHIER)
			projections {
				property('user.id')
			}
		} as List

		List<AppUser> armsUserList = appUserService.findAllByCompanyIdAndEnabledAndIdInList(companyId, true, userIds)
		return armsUserList
	}
	 ...
	 * @return - html string for select
	 */
	private String buildDropDown(List<AppUser> lstAgent, Map dropDownAttributes) {
		// read map values
		...
		{
			lstAgent = Tools.listForKendoDropdown(lstAgent,USERNAME,"All Cashier")
		}
