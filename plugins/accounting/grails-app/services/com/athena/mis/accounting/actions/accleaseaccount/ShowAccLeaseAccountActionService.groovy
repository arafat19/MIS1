...
/**
 *  Show UI for accLeaseAccount CRUD and list of accLeaseAccount for grid
 *  For details go through Use-Case doc named 'ShowAccLeaseAccountActionService'
 */
class ShowAccLeaseAccountActionService extends BaseService implements ActionIntf {
    ...
    private static final String PAGE_LOAD_ERROR_MESSAGE = "Failed to load Lease Account page"
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            Map accLeaseAccountReturn = listForAccLeaseAccount()
            List<GroovyRowResult> leaseAccountList = (List<GroovyRowResult>) accLeaseAccountReturn.accLeaseAccountList
            int count = (int) accLeaseAccountReturn.count

            result.put(Tools.COUNT, count)
            result.put(ACC_LEASE_ACCOUNT_LIST, leaseAccountList)
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            ...
            List leaseAccountList = (List) executeResult.get(ACC_LEASE_ACCOUNT_LIST)
            int count = (int) executeResult.get(Tools.COUNT)

            List wrappedLeaseList = wrapListInGridEntityList(leaseAccountList, start)
     ...
     */
    private List wrapListInGridEntityList(List<GroovyRowResult> lstAccLease, int start) {
        List accLeaseList = []
        int counter = start + 1
        for (int i = 0; i < lstAccLease.size(); i++) {
            GridEntity obj = new GridEntity()
            GroovyRowResult singleRow = lstAccLease[i]
            obj.id = singleRow.id
            obj.cell = [
                    counter,
                    singleRow.id,
     ...
     */
    private Map listForAccLeaseAccount() {
        String queryStr = """
