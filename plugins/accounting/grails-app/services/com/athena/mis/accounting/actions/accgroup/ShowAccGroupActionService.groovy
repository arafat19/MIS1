...
/**
 *  Show UI for accGroup CRUD and list of accGroup for grid
 *  For details go through Use-Case doc named 'ShowAccGroupActionService'
 */
class ShowAccGroupActionService extends BaseService implements ActionIntf {
    ...
    private static final String DEFAULT_ERROR_MESSAGE = "Failed to load account group page"
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            initPager(params)
            int count = accGroupCacheUtility.count()
            List accGroupList = accGroupCacheUtility.list(this)
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            ...
            List accGroupList = (List) executeResult.get(ACC_GROUP_LIST)
            int count = (int) executeResult.get(COUNT)
            List resultList = wrapListInGridEntityList(accGroupList, start)
            Map output = [page: pageNumber, total: count, rows: resultList]

            result.put(ACC_GROUP_LIST, output)
            result.put(Tools.IS_ERROR, Boolean.FALSE)
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
