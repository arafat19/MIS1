...
class GetDropDownInventoryOutItemTagLibActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            String html = buildDropDown(materialList, dropDownAttributes)
            return html
        } catch (Exception e) {
            log.error(e.message)
     ...
     * @return - html string for select
     */
    private String buildDropDown(List<GroovyRowResult> lstItem, Map dropDownAttributes) {
        // read map values
    ...
    private void modifyItemList(List<GroovyRowResult> lstItem, long itemId, double quantity) {
        ...
        for (int i = 0; i < lstItem.size(); i++) {
            ...
            if (itemId == eachRow.id) {
                eachRow.curr_quantity = eachRow.curr_quantity + quantity
                isItemExists = true
                break
            }
            i++
        ...
        if (!isItemExists) {
            ...
            lstItem << [id: item.id, name: item.name, unit: item.unit, curr_quantity: quantity]
        }
    }
}
