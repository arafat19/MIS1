...
class MoveSarbTaskForCancelActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            SystemEntity movedForCancel = (SystemEntity) sarbTaskReviseStatusCacheUtility.readByReservedAndCompany(SarbTaskReviseStatusCacheUtility.MOVED_FOR_CANCEL, sarbSessionUtil.appSessionUtil.getCompanyId())
            SarbTaskDetails sarbTaskDetails = sarbTaskDetailsService.findByTaskIdAndEnabled(sarbTaskModel.id)
            sarbTaskDetailsService.updateForMove(sarbTaskDetails, movedForCancel.id)
            result.put(Tools.IS_ERROR, Boolean.FALSE)
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
