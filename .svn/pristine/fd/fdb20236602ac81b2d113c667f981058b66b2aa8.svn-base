...
/*Renders html of 'select' for DocCategory*/

class GetDropDownDocCategoryTagLibActionService extends BaseService implements ActionIntf {
    ...
    public Object executePreCondition(Object parameters, Object obj) {
        ...
        try {
            ...
            if (attrs.defaultValue) {
                ...
                dropDownAttributes.put(DEFAULT_VALUE, (defaultValue > 0) ? new Long(defaultValue) : null)
            }
            if (attrs.required) {
    ...
    /**
     * Get the list of DocCategory objects by entity type id and entity id
    ...
    public Object execute(Object parameters, Object obj) {
        try {
            ...
            List<GroovyRowResult> lstDocCategory = (List<GroovyRowResult>) lstDocCategory()
            String html = buildDropDown(lstDocCategory, dropDownAttributes)
            return html
        } catch (Exception e) {
            log.error(e.message)
    ...
    private List<GroovyRowResult> lstDocCategory() {
        ...
        List<GroovyRowResult> lstDocCategory = executeSelectSql(queryForList, queryParams)
     ...
     * @param lstDocCategory - list for select 'options'
     * @param dropDownAttributes - a map containing the attributes of drop down
     * @return - html string for select
     */
    private String buildDropDown(List<GroovyRowResult> lstDocCategory, Map dropDownAttributes) {
        // read map values
        ...
        if (showHints.booleanValue()) {
            lstDocCategory = Tools.listForKendoDropdown(lstDocCategory, null, hintsText)
            // the null parameter indicates that dataTextField is name
        }
        String jsonData = lstDocCategory as JSON
