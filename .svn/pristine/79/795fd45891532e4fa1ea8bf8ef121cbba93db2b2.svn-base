...
/**
 * List of QS Measurement
 * For details go through Use-Case doc named 'ListQsMeasurementActionService'
 */
class ListQsMeasurementActionService extends BaseService implements ActionIntf {
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            if (lstProjectIds.size() > 0) {
                ...
                total = QsMeasurementModel.listByProjectIdsAndIsGovt(lstProjectIds, isGovtQs).count()
            }
    ...
    /**
     *
     * @param QSMListModel - qsm list
     * @param start - starting point of index
     * @return - wrapped qsm list
     */
    private List wrapQsMeasurement(List<QsMeasurementModel> QSMListModel, int start) {
        List lstQsMeasurement = []
        int counter = start + 1
        QsMeasurementModel qsMeasurement
        GridEntity object
        for (int i = 0; i < QSMListModel.size(); i++) {
            qsMeasurement = QSMListModel[i]
