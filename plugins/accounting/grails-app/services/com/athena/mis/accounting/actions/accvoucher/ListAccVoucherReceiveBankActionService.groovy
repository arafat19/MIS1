...
/**
 *  Show list of acc voucher receive bank(cheque) for grid
 *  For details go through Use-Case doc named 'ListAccVoucherReceiveBankActionService'
 */
class ListAccVoucherReceiveBankActionService extends BaseService implements ActionIntf {
    ...
    /**
     * Get acc voucher list of receive bank(cheque) for grid
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            SystemEntity voucherTypeObj = (SystemEntity) accVoucherTypeCacheUtility.readByReservedAndCompany(accVoucherTypeCacheUtility.RECEIVED_VOUCHER_BANK_ID, companyId)

            // Get voucher list of receive bank(cheque) by using dynamic finder
            List<AccVoucher> voucherList = AccVoucher.findAllByVoucherTypeIdAndCompanyId(voucherTypeObj.id, companyId, [offset: start, max: resultPerPage, sort: sortColumn, order: sortOrder, readOnly: true])
            int total = (int) AccVoucher.countByVoucherTypeIdAndCompanyId(voucherTypeObj.id, companyId)
    ...
    /**
     * Wrap acc voucher list of receive bank(cheque) for grid
    ...
    /**
     * Wrap list of acc voucher for receive bank(cheque) in grid entity
     * @param voucherList -list of acc voucher for receive bank(cheque) object(s)
     * @param start -starting index of the page
     * @return -list of wrapped accVouchers receive bank(cheque)
     */
