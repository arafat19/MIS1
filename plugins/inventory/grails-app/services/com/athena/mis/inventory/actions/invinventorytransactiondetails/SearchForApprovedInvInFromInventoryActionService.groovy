...
/**
 *  Get specific list of approved inventory transaction details(Inventory In From Inventory) for grid through search
 *  For details go through Use-Case doc named 'SearchForApprovedInvInFromInventoryActionService'
 */
class SearchForApprovedInvInFromInventoryActionService extends BaseService implements ActionIntf {
    ...
    /**
     * Get approved inventory transaction details(Inventory In From Inventory) list for grid through specific search
    ...
    public Object execute(Object params, Object obj) {
        ...
        try {
            ...
            int total = 0
            // get list and count of approved inventory transaction details(Inventory In From Inventory)
            long inventoryTransactionId = Long.parseLong(parameterMap.inventoryTransactionId.toString())
            if (inventoryTransactionId > 0) {
                LinkedHashMap serviceReturn = searchForApprovedInvInDetailsByInvTransId(inventoryTransactionId)
    ...
    /**
     * Get list and count of approved inventory transactions details(Inventory In From Inventory) by specific search key
     ...
     */
    private Map searchForApprovedInvInDetailsByInvTransId(long inventoryTransactionId) {
        ...
        SELECT iitd.id, item.name AS item_name, item.unit, iitd.rate,
               ...
               to_char(iitd.shrinkage,'${Tools.DB_QUANTITY_FORMAT}') shrinkage,
               to_char(iitd.approved_on,'dd-Mon-yyyy') as approved_on,
