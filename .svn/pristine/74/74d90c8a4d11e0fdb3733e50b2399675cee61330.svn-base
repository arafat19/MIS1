 ...
 * For details go through Use-Case doc named 'ListForTrialBalanceOfLevel3ActionService'
 */
class ListForTrialBalanceOfLevel3ActionService extends BaseService implements ActionIntf {

    private Logger log = Logger.getLogger(getClass())

    private static final String FAILURE_MSG = "Fail to generate trial balance"
    ...
    /**
     * Get trial balance level 3 list
     * @param parameters - serialized parameters from UI
     * @param obj -N/A
     * @return - a map containing trial balance level 3 for grid and error msg(True/False)
     */
    ...
    public Object execute(Object parameters, Object obj) {
        ...
        try {
            ...
            List<GroovyRowResult> returnTrailBalanceList = getTrialBalanceListOfLevel3(fromDate, toDate, lstProjectIds, postedByParam, divisionId)
     ...
     * @return - a map containing wrapped trial balance level 3 for grid and error msg(True/False)
     */
    ...
    /**
     * Wrap trial balance level 3 list for grid
    ...
    private List wrapTrialBalanceListInGridEntityList(List<GroovyRowResult> trialBalanceList) {
        ...
        GroovyRowResult eachObject
        GridEntity object
        for (int i = 0; i < trialBalanceList.size(); i++) {
            eachObject = trialBalanceList[i]
            object = new GridEntity()
            object.id = counter
            object.cell = [
                    counter,
                    eachObject.description,
                    eachObject.str_dr_balance,
                    eachObject.str_cr_balance
            ]
            lstTrialBalance << object
            counter++

            totalSumOfDebit = totalSumOfDebit + eachObject.dr_balance
            totalSumOfCredit = totalSumOfCredit + eachObject.cr_balance
        }
     ...
     * @return - list for trial balance level 3
     */
    private List<GroovyRowResult> getTrialBalanceListOfLevel3(Date fromDate, Date toDate, List<Long> projectIdList,
        ...
        String strQuery = """
            ...
            AND vw_voucher_details.company_id = :companyId
            AND coa.tier2 > 0
            GROUP BY coa.acc_type_id,t2.name
            ORDER BY t2.name)
