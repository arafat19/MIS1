...
/**
 *  Show list of inventory transaction(Inventory In From Supplier) for grid
 *  For details go through Use-Case doc named 'ListForInventoryInFromSupplierActionService'
 */
class ListForInventoryInFromSupplierActionService extends BaseService implements ActionIntf {
    ...
    /**
     * Get inventory transaction list (Inventory In From Supplier) for grid
    ...
    public Object buildSuccessResultForUI(Object obj) {
        ...
        try {
            ...
            List lstWrappedInventoryIn = wrapInventoryInList(lstInventoryIn, start)
            result = [page: pageNumber, total: count, rows: lstWrappedInventoryIn]
            return result
        } catch (Exception ex) {
            log.error(ex.getMessage())
            result.put(Tools.IS_ERROR, Boolean.TRUE)
    ...
    private List wrapInventoryInList(List<GroovyRowResult> lstInventoryIn, int start) {
        ...
        for (int i = 0; i < lstInventoryIn.size(); i++) {
            GroovyRowResult invInventoryIn = lstInventoryIn[i]
