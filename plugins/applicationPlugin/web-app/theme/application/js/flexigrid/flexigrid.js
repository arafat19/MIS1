...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 currentCount:false,
             flexDate:false, // Azam on Sept 15
             initHidden:false,
            startIndex:0,
             afterAjax:false, // Azam on Mar 08, 2011
             customPopulate:false,   // used to populate grid manually
            isSearchOpen:false      // used to open search box by default
		  }, p);
		...
		var g = {
			...
			addData: function (data) { //parse data
                ...
                p.currentCount = p.rp;
				p.startIndex = data.rows[0].cell[0];
				if (p.dataType=='xml')
					 ...
					 function(i,row) 
					 	{
                            ...
                            if(data.link == true){
                              if (i % 2 && p.striped) tr.className = 'erow flexiGridLink';
                              else  tr.className = 'flexiGridLink';
                            }else{
                ...
                // Azam on Sept 15, 2010
                if (p.flexDate == true) {
                    try {
                        $('input.flexDate').mask("99/99/9999");
                    } catch (e){}
                }
			...
			changeSort: function(th) { //change sortorder
                ...
                // no sorting on blank grid, Azam Mar 21, 2011
                try {
                    var hasData = (parseInt(p.total) > 0 && p.url != false);
                    if (!hasData) {
                        return;
                    }
                } catch (e) {
			...
			populate: function () { //get latest data
                ...
                // check the url before any submit
                ...
                // check the url again before proceed
				if (!p.url) return false;
				...
				var param = [
					...
					,{ name : 'currentCount', value : p.currentCount}
				];
				...
				if (p.params)
					...
					$.ajax({
					   ...
					   success: function(data){
                           if (p.customPopulate){
                              p.customPopulate(data);
                           }else{
                            g.addData(data);
                           }
                            if (p.afterAjax) p.afterAjax();
                       },
					   error: function(XMLHttpRequest, textStatus, errorThrown) { try { if (p.onError) p.onError(XMLHttpRequest, textStatus, errorThrown); if (p.afterAjax) p.afterAjax(XMLHttpRequest, textStatus); } catch (e) {} }
					 });
			},
			doSearch: function () {
				p.query =$.trim($('input[name=q]',g.sDiv).val());   // Bappy:  add trim on query
							...
							.click(
								function (e) 
									{ 
                                        ...
                                        // todo-Azam to fix an interim bug
                                        if ($(this).hasClass('bankHeader') == false) {
			...
			for (i=0;i<p.colModel.length;i++)
				{
                    ...
                    // fixing IE problem, Azam Sept, 2010
                    if (undefined === cm) {
                        break;
                    }
        ...
        if (p.width!='auto') {
            // ie6 treatment for width, Azam Sept 26, 2010
            if (! $.browser.msie) {
        ...
        if (p.isSearchOpen){       // remain open the serach box
            $(g.sDiv).show();
        }
        ...
        // Azam - to hide initially some button whose classes are pre-defined
        if (p.initHidden != false) {
            $(p.initHidden).each (function () {
                $('span.' + this).hide();
            });
        }
