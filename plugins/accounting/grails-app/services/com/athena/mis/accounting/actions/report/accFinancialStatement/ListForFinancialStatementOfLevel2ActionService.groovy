...
/**
 *  Show list of financial statement of level 2 (coa of tier1(if any) and sum of tier2) for grid
 *  For details go through Use-Case doc named 'ListForFinancialStatementOfLevel2ActionServices'
 */
class ListForFinancialStatementOfLevel2ActionService extends BaseService implements ActionIntf {
     ...
     * @param parameters - serialized parameters from UI
     * @param obj - N/A
     * @return -a map containing all financial statement list for level 2 and isError(true/false)
     */
    ...
    /**
     * Get list of financial statement of level 2 for showing in the grid
    ...
    /**
     * Wrap list of income statement of level 2 in grid entity
    ...
    private List<GroovyRowResult> getFinancialStatementList(Date fromDate, Date toDate, List<Long> projectIds, long postedByParam, long divisionId) {
        ...
        String queryStr = """
            SELECT t1.id AS id, t1.name AS description,
            CASE
            ...
            GROUP BY coa.acc_type_id, t1.name, t1.id
