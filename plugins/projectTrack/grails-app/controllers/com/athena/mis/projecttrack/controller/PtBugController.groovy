...
class PtBugController {
    ...
    static allowedMethods = [
            ...
            showBugForMyTask: "POST", selectBugForMyTask: "POST", updateBugForMyTask: "POST",
            reOpenBug: "POST", closeBug: "POST", showOrphanBug: "POST", createOrphanBug: "POST", updateOrphanBug: "POST",
            listOrphanBug: "POST", showMyBug: "POST", searchBugDetails: "POST"
    ]
    ...
    SearchPtBugDetailsActionService searchPtBugDetailsActionService
     ...
     */
    def showBugDetails() {
        Map executeResult = (Map) showPtBugDetailsActionService.execute(params, null);
        render(view: '/projectTrack/ptBug/showBugDetails', model: [result: executeResult])
    }
    /**
     * list bug by module
     */
    def bugListForModule() {
        render pt.bugList(params)
    }

    /**
     * Search Bug Details
     */
    def searchBugDetails() {
        LinkedHashMap result
        Boolean isError

        result = (LinkedHashMap) searchPtBugDetailsActionService.execute(params, null);
        isError = (Boolean) result.isError
        if (isError.booleanValue()) {
            result = (LinkedHashMap) searchPtBugDetailsActionService.buildFailureResultForUI(result)
        }

        render(template: '/projectTrack/ptBug/tmpBugDetails', model: [result: result])
