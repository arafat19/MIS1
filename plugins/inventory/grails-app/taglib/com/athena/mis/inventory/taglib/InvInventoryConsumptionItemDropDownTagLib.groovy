...
import com.athena.mis.inventory.actions.taglib.GetDropDownInventoryConsumptionItemTagLibActionService
import com.athena.mis.utility.Tools

class InvInventoryConsumptionItemDropDownTagLib {

    static namespace = "inv"

    GetDropDownInventoryConsumptionItemTagLibActionService getDropDownInventoryConsumptionItemTagLibActionService
    /**
     * Render html select of inventory item for the given transaction id
     * example: <inv:dropDownInventoryItemConsumption transactionId="${transactionId}"></inv:dropDownInventoryItemConsumption>
     *
     ...
     */
    def dropDownInventoryItemConsumption = { attrs, body ->
        Map preResult = (Map) getDropDownInventoryConsumptionItemTagLibActionService.executePreCondition(attrs, null)
        Boolean isError = preResult.isError
        if (isError.booleanValue()) {
            out << Tools.EMPTY_SPACE
            return
        }
        String html = getDropDownInventoryConsumptionItemTagLibActionService.execute(preResult, null)
        out << html
    }
